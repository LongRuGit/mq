...
int main(int argc,char *argv[])
{
    ...
    //创建队列
    if(pConsumer->CreateQueue("queue6")!=Consumer::SUCCESS)
    {
        consumerLogger.WriteLog(mq_log_err,"Create queue failed!errMsg is %s",pConsumer->GetErrMsg());
    }
    //订阅接收,自动确认
    if(pConsumer->CreateSubscribe("queue6",CONSUMER_NO_ACK)!=Consumer::SUCCESS)
    {
    ...
    if(pConsumer->DeleteQueue("queue6")!=Client::SUCCESS)
    {
